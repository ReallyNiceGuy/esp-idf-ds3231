...
#define I2C_DEV_CHECK(ARG) do { esp_err_t __CHK_RET=ARG; if (__CHK_RET!=ESP_OK) return  __CHK_RET; } while (0) 
...
esp_err_t ds3231_set_time(struct tm *time)
{
    ...
    if (I2Cdev::writeBytes(DS3231_ADDR, DS3231_ADDR_TIME, sizeof(data), data))
        return ESP_OK;
        ...
        ds3231_alarm1_rate_t option1, struct tm *time2, ds3231_alarm2_rate_t option2)
{
    ...
    I2Cdev::writeBytes(DS3231_ADDR, (alarms == DS3231_ALARM_2 ? DS3231_ADDR_ALARM2 : DS3231_ADDR_ALARM1), i, data);
 ...
 */
static esp_err_t ds3231_get_flag(uint8_t addr, uint8_t mask, uint8_t *flag)
{
    uint8_t data;

    /* get register */
    bool res = I2Cdev::readByte(DS3231_ADDR, addr, &data);
    if (!res)
        return ESP_FAIL;
 ...
 * returns true to indicate success
 */
static esp_err_t ds3231_set_flag(uint8_t addr, uint8_t bits, uint8_t mode)
{
    uint8_t data;
    ...
    res = I2Cdev::writeByte(DS3231_ADDR, addr, data);
    if (!res)
        return ESP_FAIL;
    return ESP_OK;
}
...
esp_err_t ds3231_get_oscillator_stop_flag(bool *flag)
{
    ...
    I2C_DEV_CHECK(ds3231_get_flag(DS3231_ADDR_STATUS, DS3231_STAT_OSCILLATOR, &f));
...
esp_err_t ds3231_clear_oscillator_stop_flag()
{
    ...
    I2C_DEV_CHECK(ds3231_set_flag(DS3231_ADDR_STATUS, DS3231_STAT_OSCILLATOR, DS3231_CLEAR));
...
esp_err_t ds3231_get_alarm_flags(ds3231_alarm_t *alarms)
{
    ...
    I2C_DEV_CHECK(ds3231_get_flag(DS3231_ADDR_STATUS, DS3231_ALARM_BOTH, (uint8_t *)alarms));
...
esp_err_t ds3231_clear_alarm_flags(ds3231_alarm_t alarms)
{
    ...
    I2C_DEV_CHECK(ds3231_set_flag(DS3231_ADDR_STATUS, alarms, DS3231_CLEAR));
...
esp_err_t ds3231_enable_alarm_ints(ds3231_alarm_t alarms)
{
    ...
    I2C_DEV_CHECK(ds3231_set_flag(DS3231_ADDR_CONTROL, DS3231_CTRL_ALARM_INTS | alarms, DS3231_SET));
...
esp_err_t ds3231_disable_alarm_ints(ds3231_alarm_t alarms)
{
     ...
     */
    I2C_DEV_CHECK(ds3231_set_flag(DS3231_ADDR_CONTROL, alarms, DS3231_CLEAR));
...
esp_err_t ds3231_enable_32khz()
{
    ...
    I2C_DEV_CHECK(ds3231_set_flag(DS3231_ADDR_STATUS, DS3231_STAT_32KHZ, DS3231_SET));
...
esp_err_t ds3231_disable_32khz()
{
    ...
    I2C_DEV_CHECK(ds3231_set_flag(DS3231_ADDR_STATUS, DS3231_STAT_32KHZ, DS3231_CLEAR));
...
esp_err_t ds3231_enable_squarewave()
{
    ...
    I2C_DEV_CHECK(ds3231_set_flag(DS3231_ADDR_CONTROL, DS3231_CTRL_ALARM_INTS, DS3231_CLEAR));
...
esp_err_t ds3231_disable_squarewave()
{
    ...
    I2C_DEV_CHECK(ds3231_set_flag(DS3231_ADDR_CONTROL, DS3231_CTRL_ALARM_INTS, DS3231_SET));
...
esp_err_t ds3231_set_squarewave_freq(ds3231_sqwave_freq_t freq)
{
    ...
    I2C_DEV_CHECK(ds3231_get_flag(DS3231_ADDR_CONTROL, 0xff, &flag));
    flag &= ~DS3231_SQWAVE_8192HZ;
    flag |= freq;
    I2C_DEV_CHECK(ds3231_set_flag(DS3231_ADDR_CONTROL, flag, DS3231_REPLACE));
...
esp_err_t ds3231_get_raw_temp(int16_t *temp)
{
    ...
    if(!I2Cdev::readBytes(DS3231_ADDR,DS3231_ADDR_TEMP, sizeof(data),data))
        return ESP_FAIL;
...
esp_err_t ds3231_get_time(struct tm *time)
{
    ...
    /* read time */
    if(!I2Cdev::readBytes(DS3231_ADDR, DS3231_ADDR_TIME, sizeof(data), data))
        return ESP_FAIL;
